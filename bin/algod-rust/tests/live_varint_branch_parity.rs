...
//! @ v5.0.2-stable, `data/transactions/logic/eval.go` / `assembler.go`) as
...
//!     *real* go-algorand v5.0.2-stable node's own `POST /v2/teal/compile`
...
//! v5.0.2-stable's `ConsensusCurrentVersion` V42 (`LogicSigVersion` 13) --
...
//! by contrast, already boots a *real* go-algorand v5.0.2-stable node and
...
// ---------------------------------------------------------------------------
// 1. Assembler byte-for-byte diff against a real go-algorand v5.0.2-stable
