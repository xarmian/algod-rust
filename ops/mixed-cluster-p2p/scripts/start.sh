...
if [ ! -f "$NETROOT/network.json" ] || [ ! -d "$NETROOT/Node1" ] || [ ! -d "$NETROOT/Node4Rust" ]; then
    ...
    # them open (see issue #1560, fixed for ops/mixed-cluster/ by #1562)
    # — so fix the tree up to be host-writable right away, the same way
    # the purge step above already has to reach into a container to deal
    # with this same uid-1001 ownership.
    MSYS_NO_PATHCONV=1 docker run --rm \
        -v "$(host_path "$NETROOT"):/netroot" \
        --entrypoint sh \
        "$ALGOD_IMG" \
