#!/usr/bin/env python3
# Issue #594 — per-round metric collector for the P2P mixed cluster
# (ops/mixed-cluster-p2p/), the libp2p-transport analogue of
# `ops/mixed-cluster/scripts/metrics.py` (PLAN-32 / TASK-87). Same
# record shapes and JSONL/summary convention — see
# `docs/SOAK_METHODOLOGY.md` for the format this mirrors and
# `docs/P2P_SOAK_METHODOLOGY.md` for what's different about this harness.
#
# Deviation from the WS-gossip original (noted here and in
# docs/P2P_SOAK_METHODOLOGY.md, not silently): `rust-node-4` in THIS
# harness already serves a real algod v2 REST API on host port 5004 (see
# `ops/mixed-cluster-p2p/scripts/status.sh` and
# `ops/mixed-cluster-p2p/scripts/consensus-round-trip.sh`, both of which
# already poll it exactly like a Go node), so it is included directly in
# NODES_REST here instead of being relegated to the container-state /
# log-scrape fallback the WS-gossip `metrics.py` still uses for its own
# rust-node-4 (see that script's module docstring and
# docs/SOAK_METHODOLOGY.md's "Known limitations" — `metrics.py still
# samples the Rust node by container state ... extending metrics.py to do
# the same is a small follow-up"). The container-state sample and
# `/v2/participation/status` polling are kept regardless, as additional
# liveness/participation evidence, not as the round source.
#
...
#
# Design notes (same as the WS-gossip original):
...
NODES_REST = [
    {"name": "go-node-1", "container": "p2pinterop-go-node-1", "host": "127.0.0.1", "port": 5001},
    {"name": "go-node-2", "container": "p2pinterop-go-node-2", "host": "127.0.0.1", "port": 5002},
    {"name": "go-node-3", "container": "p2pinterop-go-node-3", "host": "127.0.0.1", "port": 5003},
    {"name": "rust-node-4", "container": "p2pinterop-rust-node-4", "host": "127.0.0.1", "port": 5004},
]

# Container-liveness / best-effort log-round sample for the Rust node,
# kept as extra evidence alongside its REST-sourced round above (see the
# module docstring for why this harness can afford to treat it as a full
# REST node unlike the WS-gossip original).
NODES_NOREST = [
    {"name": "rust-node-4", "container": "p2pinterop-rust-node-4"},
]

# Nodes exposing `GET /v2/participation/status` (issue #473). Only the Rust
# node implements it — go-algorand has no equivalent endpoint.
NODES_PARTICIPATION = [
    {"name": "rust-node-4", "host": "127.0.0.1", "port": 5004},
]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="P2P mixed-cluster soak-run metric collector (issue #594)",
    )
    parser.add_argument(
