...
Ongoing maintenance phase, run whenever the pinned go-algorand reference version advances. Each sweep is its own epic (see `docs/epics/Epic-19-Go-Algorand-v4.6.0-Parity.md` for the first instance, `v4.5.1-stable` → `v4.6.0-stable`; `docs/epics/Epic-20-Go-Algorand-v4.7.0-Parity-And-P2P.md` for the second, `v4.6.0-stable` → `v4.7.0-stable`, also filed under its own `docs/PHASE10_PROPOSAL.md`; `docs/epics/Epic-21-Go-Algorand-v4.7.2-Parity.md` for the third, `v4.7.0-stable` → `v4.7.2-stable`, filed under `docs/PHASE11_PROPOSAL.md`; `docs/epics/Epic-22-Go-Algorand-v4.7.3-Parity.md` for the fourth, `v4.7.2-stable` → `v4.7.3-stable`, filed under `docs/PHASE12_PROPOSAL.md`; `docs/epics/Epic-23-Go-Algorand-v4.7.4-Parity.md` for the fifth, `v4.7.3-stable` → `v4.7.4-stable`, filed under `docs/PHASE13_PROPOSAL.md`; `docs/epics/Epic-24-Go-Algorand-v5.0.0-Parity.md` for the sixth, `v4.7.4-stable` → `v5.0.0-stable`, filed under `docs/PHASE14_PROPOSAL.md`; `docs/epics/Epic-28-Go-Algorand-v5.0.1-Parity.md` for the seventh, `v5.0.0-stable` → `v5.0.1-stable`, filed under `docs/PHASE18_PROPOSAL.md`; `docs/epics/Epic-29-Go-Algorand-v5.0.2-Parity.md` for the eighth, `v5.0.1-stable` → `v5.0.2-stable`, filed under `docs/PHASE19_PROPOSAL.md`): analyze every upstream change since the last pin, classify it (consensus-critical / api / avm / network / behavioral-other / not-applicable), open one issue per feature-level change, implement, and re-pin. See `docs/PHASE9_PROPOSAL.md`/`docs/PHASE10_PROPOSAL.md`/`docs/PHASE11_PROPOSAL.md`/`docs/PHASE12_PROPOSAL.md`/`docs/PHASE13_PROPOSAL.md`/`docs/PHASE14_PROPOSAL.md`/`docs/PHASE18_PROPOSAL.md`/`docs/PHASE19_PROPOSAL.md` and the `algod-version-upgrade` skill for the full process. Does not gate on or block Phase 7/8 — orthogonal maintenance work, can interleave with either.
...
The `v4.7.4-stable → v5.0.0-stable` sweep is a consensus-upgrade release (V41 → V42): native post-quantum (Falcon-1024) account signatures, big-transaction size pricing (per-byte fee surcharge, larger notes/args/program-pages/LogicSigs), AVM v13 (poseidon2, app_params_set, foreign-box opcodes, variable-length branches, auto-salted LogicSig addresses), a new `SelectF128` 128-bit-software-float sortition/committee-selection algorithm (the epic's highest consensus risk, live-verified via a 30-round mixed-cluster soak), a new `GET /v2/node/peers` endpoint, and removal of the `dryrun` REST endpoint. Complete — see `docs/PHASE14_VALIDATION.md` for the evidence map.

The `v5.0.0-stable → v5.0.1-stable` sweep is a small security/safety patch release with no consensus version bump: the state proof verifier now pins Merkle proofs to the protocol-fixed hash algorithm and rejects wrong-sized proof-path elements instead of silently zero-padding/truncating them. See `docs/PHASE18_PROPOSAL.md` and epic issue [#1537](https://github.com/xarmian/algod-rust/issues/1537).

The `v5.0.1-stable → v5.0.2-stable` sweep is a small safety/durability patch release with no consensus version bump: three agreement-layer hardening changes against untrusted network input — a stateless bound check rejecting any vote/bundle/proposal step above `down`, isolating bundle-verification crypto contexts to be round-scoped only (never keyed by a bundle's own unauthenticated claimed period), and removing the `cert`-step exemption from bundle freshness checking so stale cert bundles are discarded like any other bundle. See `docs/PHASE19_PROPOSAL.md` and epic issue [#1547](https://github.com/xarmian/algod-rust/issues/1547).

---

## Phase 15 — Licensing and Legal-Framework Compliance

Brings algod-rust into full compliance with the legal framework it operates under. go-algorand's node software is AGPL-3.0-or-later with section 7e Additional Terms, and its `COPYING_FAQ` states that reimplementing the node/APIs/consensus automatically produces an AGPL-licensed work — which is exactly what algod-rust is. The project-owner decisions this phase implements: algod-rust as a whole is classified as a **modified work based on go-algorand under AGPL-3.0-or-later** (preserving the inherited section 7e trademark terms); **MIT is preferred wherever legally possible** for files not derived from AGPL material; the legal entity for all algod-rust copyright/attribution statements is **`Algod DAO`**. Scope: full per-file AGPL/MIT/third-party audit, repo-level `COPYING`/`LICENSE-MIT`, per-file modified-work headers with SPDX identifiers, `Cargo.toml` license metadata, `docs/LICENSING.md` (AGPL section 13 network-source obligation, trademark posture, patent rationale, third-party attributions such as gnark-crypto's Apache-2.0 poseidon2), and updates to `CLAUDE.md`/`.claude/skills/*` so future files get correct headers by default. Complete — 637 source/config files headered as pure insertions across three sweeps, `Cargo.toml` license metadata on all 27 crates, a CI license-header-presence check plus a `cargo-deny` dependency-license gate, and an in-node AGPL section 13 source-availability pointer (`X-Algod-Rust-Source` response header + startup banner). See `docs/PHASE15_VALIDATION.md` for the evidence map, `docs/PHASE15_PROPOSAL.md`, `docs/epics/Epic-25-Licensing-Compliance.md`, and epic issue [#732](https://github.com/xarmian/algod-rust/issues/732).

---

## Phase 16 — Node Configuration and Consensus-Parameter Parity

Audited algod-rust's node-configuration surface and consensus-parameter struct against go-algorand's, field by field, and closed every real divergence found — unlike phases 9–14, which only tracked *version-delta* parity (what changed between two pins) and had no reason to catch a gap present since the original phase 0–3 port. Headline findings at the start of the phase: algod-rust had no `config.json` ingestion mechanism at all (go-algorand's ~97 real `Local` fields were ~9 matching, ~20 different, ~10 not-applicable, and ~58 with no equivalent); 23 consensus-parameter fields were absent from `consensus.rs` (8 self-documented as skipped, 15 silently missing — several were historical-replay-correctness risks, i.e. bugfix-activation flags that must be version-gated rather than applied unconditionally); and no custom consensus.json load/merge/save mechanism existed, even though a real go-algorand-authored one already sat unused in the repo (`docker/config/vfuture-consensus.json`). Complete — 30 sub-issues closed (12 originally scoped plus 18 follow-ups filed as gaps surfaced, none disposed as unreachable/deferred): a full `config.json` loading/migration mechanism (`config.Local` equivalent) with per-area field parity across networking, storage, REST/API, catchup/sync, agreement, and telemetry/metrics; a custom consensus.json load/merge/save mechanism threaded through every `consensus_params_for_version` call site and live-verified over the wire; all 23 previously-missing `ConsensusParams` fields, including a from-scratch independent state-proof voter-set formation/verification implementation; and a fully wired transaction-sync loop plus automatic catchpoint generation, both with go-algorand-matching wire/file-format fidelity and graceful-shutdown/atomicity guarantees. See `docs/PHASE16_VALIDATION.md` for the evidence map, `docs/PHASE16_PROPOSAL.md`, `docs/epics/Epic-26-Configuration-Consensus-Parity.md`, and epic issue [#745](https://github.com/xarmian/algod-rust/issues/745).
