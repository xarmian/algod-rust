...
The `v4.7.4-stable → v5.0.0-stable` sweep is a consensus-upgrade release (V41 → V42): native post-quantum (Falcon-1024) account signatures, big-transaction size pricing (per-byte fee surcharge, larger notes/args/program-pages/LogicSigs), AVM v13 (poseidon2, app_params_set, foreign-box opcodes, variable-length branches, auto-salted LogicSig addresses), a new `SelectF128` 128-bit-software-float sortition/committee-selection algorithm (the epic's highest consensus risk), a new `GET /v2/node/peers` endpoint, and removal of the `dryrun` REST endpoint. See `docs/epics/Epic-24-Go-Algorand-v5.0.0-Parity.md` and epic issue [#678](https://github.com/xarmian/algod-rust/issues/678) for the full sub-issue list.
