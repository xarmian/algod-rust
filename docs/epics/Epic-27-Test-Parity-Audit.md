...
- [ ] #829 — avm: implement static stack-type-tracking pass in the TEAL assembler; slice 1 (straight-line/branch-free tracking) via PR #883; slice 2 (branch-merge type unification, ported go's `deadcode`/`bottom`/`deadens` algorithm) via PR #892; slice 3 (scratch-slot per-index type tracking) via PR #899; slice 4 (`#pragma typetrack false`/`true` toggling) via PR #903, completing the rest of `TestTypeTracking`; dynamic-/arity-dependent opcodes (match/txn/gtxn/gtxns/popn/dupn/cover/uncover/pushbytess/pushints) and bounds-refined types remain as documented follow-up work
