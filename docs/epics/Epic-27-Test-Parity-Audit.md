...
- [ ] #888 — tools: goal-rust `app call`/`app method` ABI CLI subcommand (split from #820) — needs a materially larger ARC-4/ABI-encoding subsystem (method-selector computation, JSON arg parsing, tx construction/signing/submission) that doesn't exist yet
