...
- [ ] #825 — testing: close agreement missing-test gaps; theme 4's `TestProposalCarriesOversizedTxnGroup` fixed via PR #857 (a real bug, not just a missing test), `TestProposalManagerRejectsUnknownEvent` via PR #858, `TestSampleIndexIsValid`/`TestLowerBound` constant invariants via PR #859; theme 2 partial (5 of 14 credential-arrival-history scenarios) via PR #868 — found+fixed a real bug (`validated_at` was always `Duration::ZERO`, silently defeating the dynamic-filter-timeout feature); theme 1 partial (6 named regressions + 2 network-hardening scenarios) via PR #876. `TestSortProposalValueLess`, `TestPseudonodeNonEnqueuedTasks`, theme 1's offset-start/late-proposal/ISV-ICV/pipelined-threshold scenarios, theme 2's remaining 9 scenarios, and theme 3 (service-level fast-recovery) remain open
