...
- [ ] #888 — tools: goal-rust `app call`/`app method` ABI CLI subcommand (split from #820); slice 1 (PR #896) adds the ARC-4 ABI encoding subsystem itself — new `crates/core/algo-abi` crate (extracted from `algo-rest-api`'s private `abi.rs`), method-selector computation, `Method`/Contract-JSON signature parsing, ABI value decode, JSON display formatting, and go's more-than-15-arguments tuple-bundling rule — verified against real ARC-4 vectors from go-algorand's own e2e ABI fixture and `TestParseMethodArgJSONtoByteSlice`. Still open: the actual `goal-rust` CLI subcommand wiring (flag surface, CLI-value parsing, resource resolution via #887, tx construction/signing/submission)
