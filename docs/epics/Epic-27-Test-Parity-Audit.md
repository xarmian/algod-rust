...
- [ ] #901 — sync: wire catchup peer-selection/ranking algorithm into live block/catchpoint fetch paths (split from #819) — needs live mixed-cluster verification per this epic's safe-scoping discipline for network/sync-path wiring
- [ ] #829 — avm: implement static stack-type-tracking pass in the TEAL assembler; slice 1 (straight-line/branch-free tracking) via PR #883; slice 2 (branch-merge type unification) via PR #892; slice 3 (scratch-slot tracking) via PR #899; slice 4 (`#pragma typetrack` toggling) via PR #903; slice 5 (`popn`/`dupn`/`cover`/`uncover` arity tracking) via PR #904; slice 6 (`match`, `txn`/`gtxn`/`gtxns`/pseudo-ops, `pushbytess`/`pushints`) via PR #908, closing `TestMatchTyping` — all opcodes now modeled; only bounds-refined `[min,max]` `StackType`s (investigated, no currently-modeled opcode's verdict depends on it) and `itxn_field`'s separate field-type checking (`TestTxTypes`) remain as documented, lower-priority follow-up work
...
- [ ] #827 — testing: close REST/kmd/multi-node-cluster negative-path missing-test gaps; theme 1 (REST negative-path coverage) via PR #861; themes 2/3/5 via PR #874 (found+fixed a real msgpack decode bug: 10 `Option` fields missing `#[serde(default)]`); theme 4 (multi-node cluster) investigated via PR #910 and reconfirmed to need a genuinely different node-orchestration/daemon-level harness than the agreement-protocol-level one #825 needed — `ops/mixed-cluster/`'s docker-based harness (used live for #845/PR #909) is the closer analogue but is heavier-weight than a per-PR unit-test harness; remains genuinely open
