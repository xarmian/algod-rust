...
The go-algorand source is available at `../go-algorand`, pinned to `v4.6.0-stable`:

```bash
cd ../go-algorand
git log --oneline -1   # should show v4.6.0-stable tag
```
...
- Image: `algorand/algod:4.6.0-stable` (no `v` prefix)
...
The tool enforces the go-algorand pin (`v4.6.0-stable`) at runtime; override
...
the tag tracked in `CLAUDE.md` (currently `v4.6.0-stable`) with a clean
...
enforces the same `v4.6.0-stable` pin + clean `agreement/` tree the
...
`v4.6.0-stable` pin + clean tree the other captures do, filtering
...
pinned `go-algorand@v4.6.0-stable` `algokey` binary.
...
source tree pinned to `v4.6.0-stable`.
