...
The go-algorand source is available at `../go-algorand`, pinned to `v5.0.2-stable`:

```bash
cd ../go-algorand
git log --oneline -1   # should show v5.0.2-stable tag
```
...
- Image: `algorand/algod:5.0.0-stable` (no `v` prefix)
...
The tool enforces the go-algorand pin (`v5.0.2-stable`) at runtime; override
...
the tag tracked in `CLAUDE.md` (currently `v5.0.2-stable`) with a clean
...
enforces the same `v5.0.2-stable` pin + clean `agreement/` tree the
...
`v5.0.2-stable` pin + clean tree the other captures do, filtering
...
pinned `go-algorand@v5.0.2-stable` `algokey` binary.
...
source tree pinned to `v5.0.2-stable`.
