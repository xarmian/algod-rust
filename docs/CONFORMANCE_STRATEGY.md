...
4. A clean go-algorand v4.7.4-stable container boots against that data
   dir and must:
...
3 x algorand/algod:4.7.3-stable   relay + proposer, 30% online stake each
...
| Go verifies certificates from a Rust-participating chain | `make consensus-cluster-test` | `tools/cert-authenticate/` — a real go-algorand v4.7.4-stable binary running `agreement.Certificate.Authenticate` over ledger facts exported from the **Rust** ledger (check `certs_authenticate_go`) |
...
Byte-level differential suite vs go-algorand `v4.7.4-stable`. Target
...
- `docker/docker-compose.vfuture.yml` stands up a single-node,
  100%-own-stake go-algorand `4.7.3-stable` private network pinned to
