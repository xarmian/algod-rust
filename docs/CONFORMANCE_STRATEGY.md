...
4. A clean go-algorand v4.7.3-stable container boots against that data
   dir and must:
...
| Go verifies certificates from a Rust-participating chain | `make consensus-cluster-test` | `tools/cert-authenticate/` — a real go-algorand v4.7.3-stable binary running `agreement.Certificate.Authenticate` over ledger facts exported from the **Rust** ledger (check `certs_authenticate_go`) |
...
Byte-level differential suite vs go-algorand `v4.7.3-stable`. Target
