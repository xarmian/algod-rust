...
4. A clean go-algorand v5.0.2-stable container boots against that data
   dir and must:
...
3 x algorand/algod:5.0.2-stable   relay + proposer, 30% online stake each
...
| Go verifies certificates from a Rust-participating chain | `make consensus-cluster-test` | `tools/cert-authenticate/` — a real go-algorand v5.0.2-stable binary running `agreement.Certificate.Authenticate` over ledger facts exported from the **Rust** ledger (check `certs_authenticate_go`) |
...
Byte-level differential suite vs go-algorand `v5.0.2-stable`. Target
