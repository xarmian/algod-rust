...
| 2026-05-22 | commit | 0.97× | TASK-149 | `refurbishNode` ID recycling ported. Avoids `TrieNode` clones in `node_add`/`node_remove` CoW paths (algorithmic parity with Go). Honest measurement: incremental-commit **bytes written are unchanged** (19,268 bytes both pre- and post-PR) — the same nodes end up persisted either way. Refurbish is a heap-churn / commit-time win (~3% — within bench noise), not a bytes-on-disk win. Value of the change is correctness/parity, not perf. |
