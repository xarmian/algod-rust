...
Until PLAN-144 Task 2 (lazy on-demand page loading) lands, the `cold-load`
ratio is **not** an apples-to-apples comparison. Go's `MakeTrie` reads
only page 0 (the metadata) and defers all other node fetches to first-use
— a constant-time operation against a populated committer. Rust's
`MerkleTrie::load` eagerly reads every page and reconstructs the full
in-memory tree. The Rust cold-load number reflects the cost of
eager-loading the entire working set; the Go number reflects metadata
deserialization only. Once lazy load lands (PLAN-144 Task 2), Rust's
cold-load is expected to drop to within ~1.5× of Go.

`apply` and `commit` are directly comparable today: both implementations
do the full work in those phases.
...
|-------|-----------------:|---------------:|---------------:|--------------:|------------:|------------:|
| `apply` |         2014818 |        6720478 |          0.30× |       2036813 |    15213112 |       0.13× |
| `cold-load` |          121251 |           5856 |         20.71× |        126253 |       98653 |       1.28× |
| `commit` |          265439 |        1161850 |          0.23× |        287911 |     3647487 |       0.08× |
