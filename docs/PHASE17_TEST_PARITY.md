...
| `matched-1:many` | 484 | 15% |
| `out-of-scope` | 374 | 12% |
| `missing-test` | 354 | 11% |
| `matched-many:1` | 204 | 6% |

**874 rows (`not-implemented` + `missing-test`, 28%) are real, actionable
gaps** — either a behavior algod-rust doesn't implement yet, or one it
implements but never tests. `partial` (720, 23%) is coverage that exists
...
| Ledger core (`ledger`, `ledger/eval`, `ledger/apply`, `ledger/ledgercore`, `ledger/store`, `ledger/encoded`) | [parity_ledger_core.md](phase17/parity_ledger_core.md) | 503 | 41 | 95 | 43 | 132 | 40 | 19 | 133 |
...
| **Total** | | **3,177** | **521** | **484** | **204** | **720** | **520** | **354** | **374** |
