...
| `partial` | 743 | 23% |
| `matched-1:1` | 705 | 22% |
| `matched-1:many` | 556 | 17% |
| `out-of-scope` | 424 | 13% |
| `not-implemented` | 283 | 9% |
| `missing-test` | 260 | 8% |
| `matched-many:1` | 208 | 7% |

**543 rows (`not-implemented` + `missing-test`, 17%) are real, actionable
gaps** — either a behavior algod-rust doesn't implement yet, or one it
implements but never tests. `partial` (743, 23%) is coverage that exists
...
| Ledger core (`ledger`, `ledger/eval`, `ledger/apply`, `ledger/ledgercore`, `ledger/store`, `ledger/encoded`) | [parity_ledger_core.md](phase17/parity_ledger_core.md) | 503 | 42 | 100 | 43 | 132 | 39 | 10 | 137 |
...
| **Total** | | **3,179** | **705** | **556** | **208** | **743** | **283** | **260** | **424** |
