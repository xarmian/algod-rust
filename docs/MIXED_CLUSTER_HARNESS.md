...
- **Genesis interop — verified.** `participate --genesis-json` seeds
  ...
  `algorand/algod:5.0.1-stable` node serves from `/v2/ledger/supply` at
...
go-algorand v5.0.1-stable nodes, each started in plain P2P mode
...
go-algorand v5.0.1-stable's own `gossipSubTags` map
...
`algorand/algod:5.0.1-stable` Docker Hub image (already used by
