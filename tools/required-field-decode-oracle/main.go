...
// module) and feeds them to go-algorand v4.7.2-stable's own generated
...
func main() {
	...
	if mismatches > 0 {
		fmt.Printf("\n%d/%d cases mismatched go-algorand v4.7.2-stable's real decoder\n", mismatches, len(results))
		os.Exit(2)
	}
	fmt.Printf("\nall %d cases match go-algorand v4.7.2-stable's real decoder\n", len(results))
}
