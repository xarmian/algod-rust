...
// ---------------------------------------------------------------------------
// Test-only constants, types, and FFI (keygen + sign)
// ---------------------------------------------------------------------------
...
/// Seed size used for deterministic key generation (test-only; not needed on-chain).
#[cfg(test)]
...
// ---------------------------------------------------------------------------
// Production extern "C" declarations (verify only)
// ---------------------------------------------------------------------------
...
///
/// Only used in tests -- the on-chain opcode only needs `falcon_verify`.
#[cfg(test)]
...
mod tests {
    ...
    fn test_keygen_sign_verify_roundtrip() {
        // Use an all-zero seed (deterministic).
    ...
    fn test_deterministic_signatures() {
        // Same seed + same message should produce the same signature.
    ...
    fn test_invalid_signature_one_byte() {
        // A 1-byte signature must also be rejected (sig_len < 2 matches the
        // C library's precondition).
        let seed = [0u8; FALCON_SEED_SIZE];
        let (pubkey, _) = falcon_keygen(&seed).expect("keygen should succeed");
    ...
    /// Test using the known test vector from go-algorand's crypto_test.go
    /// (the falcon_verify opcode test). This ensures our FFI bindings produce
    /// results consistent with the Go implementation.
    #[test]
    fn test_go_algorand_test_vector() {
        // From go-algorand: seed = all-zeros, message =
        // hex "62fdfc072182654f163f5f0f9a621d729566c74d0aa413bf009c9800418c19cd"
        let seed = [0u8; FALCON_SEED_SIZE];
        let (pubkey, privkey) = falcon_keygen(&seed).expect("keygen should succeed");
