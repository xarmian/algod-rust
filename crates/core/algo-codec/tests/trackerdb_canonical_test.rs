...
fn decode_resources_data_value(data: &[u8]) -> Result<ResourcesData, String> {
    ...
    for (k, v) in pairs {
        ...
        match key {
            ...
            "d" => d.unit_name = as_str(&v)?.into_bytes(),
            "e" => d.asset_name = as_str(&v)?.into_bytes(),
            "f" => d.url = as_str(&v)?.into_bytes(),
...
fn gen_resources_data(rng: &mut ChaCha20Rng) -> ResourcesData {
    ResourcesData {
        ...
        unit_name: gen_string(rng, 8).into_bytes(),
        asset_name: gen_string(rng, 12).into_bytes(),
        url: gen_string(rng, 16).into_bytes(),
