...
- Read `../go-algorand`'s real behavior for the relevant area before forming a hypothesis — this repo's bar is byte-level/behavioral parity with go-algorand v5.0.2-stable, not "something reasonable." Cite the specific go file/function once you have a root-cause theory.
...
- **Test-parity rows are acceptance criteria too.** If the issue names go-algorand tests — a Phase 17 row (`docs/phase17/parity_<area>.md`), or a version-upgrade epic's Pass C obligation (`unclassified` placeholder rows inserted by `scripts/phase17_parity_delta.py repin`, or body-changed rows assigned to this issue) — this PR must update those rows itself: final status (`matched-1:1` / `matched-1:many` / `matched-many:1`, or an argued `out-of-scope`) with a working relative link to the Rust test, a one-sentence re-verification note on body-changed rows, then `python3 scripts/update_phase17_summary.py` so `docs/PHASE17_TEST_PARITY.md`'s tables match. A named row still `unclassified` / `missing-test` / `not-implemented` / `partial` is an **unmet criterion**, handled exactly like any other one above (met, struck with reason, or moved to a follow-up that is itself worked) — never merged over silently. `python3 scripts/phase17_parity_delta.py check` shows the current gap counts.
