...
- an **upstream-version label naming the release where the feature first appeared** — `algod:<tag>` (e.g. `algod:v4.6.0-stable`, or `algod:v4.6.0-beta.1` for a feature that only ever shipped in a pre-release before being folded into NEW); create one per tag in `TAGS_IN_RANGE` as needed (`gh label create "algod:<tag>" --description "Introduced in go-algorand <tag>" --color 8250DF`). When OLD..NEW spans several releases, do NOT blanket-tag everything with NEW, and do NOT collapse a beta/rc-only feature onto its later stable tag: find each feature's true origin tag across the **full** `TAGS_IN_RANGE` list, pre-releases included (`git -C ../go-algorand tag --contains <sha> --list | sort -V | head -1`, cross-checked against the release notes) and tag with that,
