...
- **Every open topic a merge leaves behind becomes a sub-issue of this epic.** `algod-issue-fix` step 9's pre-merge acceptance-criteria audit and open-topics sweep (deferred findings, out-of-scope bugs, introduced TODOs, admitted limitations, unmet or moved criteria) apply to every PR in this loop — and here each such new issue must additionally be **incorporated into the epic's working plan for delivery**: add it to the epic issue's dependency-ordered sub-issue list at the position its dependencies dictate (`gh issue edit <epic> --body ...`), give it the stage-3 issue template **including its own acceptance criteria** (a criterion moved out of another issue arrives here verbatim, with a back-link to where it came from), the full label set (`phase:<N>`, `algod:<tag>`, domain, effort, kind), and note in the epic which merged PR spawned it. The loop is not finished when the stage-3 list is empty — it is finished when the epic's list is empty, *including* everything merges added to it along the way.
